...
do
	...
	function section:updateTextbox(textbox, title)
	...
	function section:updateColorPicker(colorpicker, title, value)
		...
		if value then
			if typeof(value) == "table" then
				local hue, sat, brightness = unpack(value)
				local color3 = Color3.fromHSV(hue, sat, brightness)
				
				picker.tab.Container.Canvas.ImageColor3 = Color3.fromHSV(hue, 1, 1)
				picker.tab.Container.Color.Select.Position = UDim2.new(hue, 0, 0, 0)
				
				utility:Tween(picker.tab.Container.Canvas.Cursor, {Position = UDim2.new(sat, 0, 1 - brightness, 0)}, 0.1)
				
				local r, g, b = color3.R * 255, color3.G * 255, color3.B * 255
				picker.tab.Container.Inputs.R.Textbox.Text = tostring(math.floor(r))
				picker.tab.Container.Inputs.G.Textbox.Text = tostring(math.floor(g))
				picker.tab.Container.Inputs.B.Textbox.Text = tostring(math.floor(b))
			else
				local color3 = value
	...
	function section:updateSlider(slider, title, value, min, max, new)
		...
		min = min or slider.TextBox.Text
		max = max or 100
		value = value or new
		...
		if value ~= nil then
			...
			slider.Slider.Bar.Fill.Size = UDim2.new(percentage, 0, 1, 0)
			slider.TextBox.Text = tostring(value)
			
			return value
		else
			local mouseLocation = input:GetMouseLocation()
			local relativePosition = mouseLocation - slider.Slider.Bar.AbsolutePosition
			local percentage = math.clamp(relativePosition.X / slider.Slider.Bar.AbsoluteSize.X, 0, 1)
	...
	function section:updateDropdown(dropdown, title, list, callback)
		...
		if list == nil then
			dropdown.Search.Button.Rotation = 180
			utility:Tween(dropdown.List, {Size = UDim2.new(1, 0, 1, -34)}, 0.2)
			return
		end
		
		dropdown.Search.Button.Rotation = 0
		...
		local function addItem(text)
			...
			local layoutOrder = frame.UIListLayout.LayoutOrder
			frame.CanvasSize = UDim2.new(0, 0, 0, layoutOrder * 32)
		end
		...
		utility:Tween(dropdown.List, {Size = UDim2.new(1, 0, 0, math.min(#list * 32, 120))}, 0.2)
	end
