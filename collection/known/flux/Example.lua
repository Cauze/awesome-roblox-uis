...
function Flux:Window(text, bottom, mainclr, toclose)
	...
	function Flux:Notification(desc, buttontitle)
		...
		CloseBtn.Text = buttontitle or "OK"
		...
		wait(.4)
		TweenService:Create(NotificationDesc, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0.3}):Play()
		TweenService:Create(CloseBtn, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0.3}):Play()
	...
	function Tabs:Tab(text, ico)
		...
		TabIcon.Image = ico or "http://www.roblox.com/asset/?id=6034767608"
		...
		function ContainerContent:Slider(text, desc, min, max, start, callback)
			...
			ArrowBtn.MouseButton1Click:Connect(function()
				if SliderDescToggled == false then
					...
					TweenService:Create(Title, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = PresetColor}):Play()
					TweenService:Create(Value, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = PresetColor}):Play()
					TweenService:Create(ArrowIco, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageColor3 = PresetColor}):Play()
					TweenService:Create(ArrowIco, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {ImageTransparency = 0}):Play()
					TweenService:Create(ArrowIco, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {Rotation = 180}):Play()
					TweenService:Create(Circle, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = PresetColor}):Play()
		...
		function ContainerContent:Bind(text, presetbind, callback)
			...
			Bind.MouseButton1Click:connect(function()
				TweenService:Create(Title, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = PresetColor}):Play()
				TweenService:Create(BindLabel, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextColor3 = PresetColor}):Play()
				TweenService:Create(Circle, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundColor3 = PresetColor}):Play()
				TweenService:Create(CircleSmall, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
				TweenService:Create(Title, TweenInfo.new(.3, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
...
local Main = Flux:Window("Flux", "Premium UI Library", Color3.fromRGB(66, 134, 255), Enum.KeyCode.RightControl)

local Tab1 = Main:Tab("Combat")
local Tab2 = Main:Tab("Visuals")
local Tab3 = Main:Tab("Misc")
local Tab4 = Main:Tab("Settings")

Tab1:Button("Kill All", "Executes kill all function", function()
	print("Kill All executed")
end)

Tab1:Toggle("Aimbot", "Enables aimbot feature", false, function(state)
	print("Aimbot:", state)
end)

Tab1:Slider("FOV", "Sets aimbot field of view", 10, 360, 90, function(value)
	print("FOV:", value)
end)

Tab1:Bind("Silent Aim", Enum.KeyCode.G, function()
	print("Silent Aim toggled")
end)

local DropdownExample = Tab1:Dropdown("Target Part", {"Head", "Torso", "Random"}, function(selected)
	print("Target:", selected)
end)

Tab2:Toggle("ESP", "Shows player esp", true, function(state)
	print("ESP:", state)
end)

Tab2:Toggle("Tracers", "Shows player tracers", false, function(state)
	print("Tracers:", state)
end)

Tab2:Toggle("Box ESP", "Shows player boxes", false, function(state)
	print("Box ESP:", state)
end)

Tab2:Colorpicker("ESP Color", Color3.fromRGB(255, 0, 0), function(color)
	print("ESP Color:", color)
end)

Tab2:Slider("ESP Distance", "Max distance for ESP", 50, 500, 200, function(value)
	print("ESP Distance:", value)
end)

Tab3:Label("Information Label")

Tab3:Textbox("Player Name", "Enter player name to target", true, function(text)
	print("Target:", text)
end)

Tab3:Line()

Tab3:Button("TP to Player", "Teleports to targeted player", function()
	print("Teleporting...")
end)

Tab3:Button("Log Player", "Logs player info to console", function()
	print("Logging player data...")
end)

Tab4:Bind("UI Toggle", Enum.KeyCode.RightControl, function()
	print("UI Toggled")
end)

Tab4:Dropdown("Theme", {"Default", "Dark", "Light"}, function(selected)
	print("Theme:", selected)
end)

Tab4:Colorpicker("Accent Color", Color3.fromRGB(66, 134, 255), function(color)
	print("Accent Color:", color)
end)

Flux:Notification("Flux UI loaded successfully!", "OK")